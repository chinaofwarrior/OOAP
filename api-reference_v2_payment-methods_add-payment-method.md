...
Add a new payment method with billing address. The backend generates an idempotency key based on card details to prevent duplicates. If the same card is submitted multiple times, only one payment method will be created.
...
[List Payment Methods](/api-reference/v2/payment-methods/list-payment-methods)[Get payment method by ID](/api-reference/v2/payment-methods/get-payment-method-by-id)
